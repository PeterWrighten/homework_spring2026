    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                pg_loss = -masked_mean_per_row(torch.min(unclipped, clipped), mask).mean()
                kl = approx_kl_from_logprobs(new_logp, mb.ref_logprobs, mask)
                entropy = -masked_mean(new_logp, mask)
                clipped_flag = ((ratio < 1.0 - cfg.clip_eps) | (ratio > 1.0 + cfg.clip_eps)).float()
                clipfrac = masked_mean(clipped_flag, mask)
